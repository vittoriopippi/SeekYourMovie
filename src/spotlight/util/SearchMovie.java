...
public class SearchMovie {
	...
	/**
	 * Funzione che accetta una lista che viene prima trasformata in una singola
	 * stringa, poi costruito il link per accedere al server rest, parsato l'oggetto
	 * ritornato, riempito un JSONArray(Array di JSONObject) che viene infine
	 * ritornato
	 * 
	 * @param subList
	 * @param year
	 ...
	 */
	private static JSONArray ListToArray(List<String> subList, Integer year)
			throws ResourceException, IOException, ParseException {
		String tempString;
		ArrayList<String> tempList = new ArrayList<String>();
		/*
		 * aggiungo in una lista temporanea tutti gli elementi di una sottolista che
		 * comprende gli elementi che vanno dall'indice i all'indice i+dim estremi
		 * inclusi
		 */
		tempList.addAll(subList);

		/*
		 * Unisce le stringhe della tempList formando una stringa unica separata da
		 * spazi
		 */
	...
	/**
	 * Funzione che ritorna l'unione di tutti gli elementi della lista passata
	 * spearati da spazi
	 * 
	 * @param list
	 * @return String
	 ...
	 * @throws ParseException
	 */
	private static Integer ResearchMovie(RawMovie rawMovie, Integer dim)
			throws ResourceException, IOException, ParseException {
		...
		if (dim < 0)
			return null;
		/*
		 * nel caso in cui la dimensione dei segmenti è 0 e ci sono elementi nel
		 * minJArray ritorna il primo elemento dell'array
		 */
		...
		for (Integer year : rawMovie.getDataList()) {
			for (int i = 0; i < size; i++) {
				if ((i + dim) > size)
					// controllo che la sottolista che andrò a creare sia contenuta nella lista
					// originaria
					break;
				...
				if (filmJSONArray.size() >= 1) {
					/*
					 * Nel caso è stato trovato un colo risultato oppure nel caso in cui sono tati
					 * trovai più di un risultato ma è la prima ricerca che si fa, questo perchè se
					 * alla prima ricerca si trovano molteplici risultati significa che levando
					 * elementi alla sottolista usata per la ricerca non si fa altro che rimuovere
					 * vincoli e quindi tanto vale ritornare il primo risultato
					 */
	...
	// System.out.println("##################################################");
	// System.out.println(Analyze.Filename(i).getTitleList().size() + "\tTitolo
	// analizzato:\t" + Analyze.Filename(i).getTitleList());
	// System.out.println("\n" + SearchMovie.TMDb(Analyze.Filename(i)));
	// }
	// elapsedTime = System.nanoTime() - startTime;
	// System.out.println("\nCercati " + dl.getFilmList().size() + " film in " +
	// elapsedTime/1000000 + " ms con una media di " +
	// elapsedTime/(1000000*dl.getFilmList().size()) + " ms ad elemento");
	// }
}
