...
public class MovieLoader implements Runnable{
	...
	public void run() {
		...
		if(Setting.TEMP_ARRAY.size() == 0){
			...
			for(String s:nff.getFilmList()){
				...
				//se sto aggiungendo un bottone che si riferisce ad un nuovo file creo il bottone altrimenti metto il bottone già creato
				if(wrap.contains(s))
		...
		for(MovieButton mButton: buttonList){
			...
			if(newDirectory == null){
				...
				if(moviePath.contains(new File(newDirectory))){
					...
					System.out.println("non è contenuto\t" + newDirectory + "\n\n" + buttonList.size() + "\n" + counter);
			...
			if(flag){ //se il film è contenuto tra i path cercati
				...
				System.out.println("Inizio ricerca del Film: " + mButton.getAbs_path().substring(mButton.getAbs_path().lastIndexOf(Sys.file_separator)+1));
				if(Check.Connection())
				...
				if(tempMovie != null){
					...
					counter++;
					BarManager.addMessage("Non trovato: " + mButton.getAbs_path().substring(mButton.getAbs_path().lastIndexOf(Sys.file_separator)+1));
				}
			}
			else {
				counter++;
	...
	private void addButton(MovieButton movieButton){
		// "Platform.runLater.." è necessario per avviare un thread che modifica la UI del programma
		// Bisogna usare il metodo "getChildrenUnmodifiable()" per evitare un eccezione
		// dovuta al fatto che il programma crede di 

		//Tutto quello che avviene dentro il "Platform.runLater" viene eseguito in un altro momento
		Platform.runLater(()->{
