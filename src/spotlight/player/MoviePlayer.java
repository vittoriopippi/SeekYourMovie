...
public class MoviePlayer {
	...
	public boolean playing = false; // permette di sapere in ogni momemnto (public) se e' in riproduzione un file
									// video
	private Process p;// permette di avviare il processo di riproduzione del file video

	/**
	 * Il costruttore, sulla base del SO corrente, seleziona il player per
	 * riprodurre i file video: WINDOWS: - VLC: C:\Program Files
	 * (x86)\VideoLAN\VLC\vlc.exe - WindowsMediaPlayer: C:\Program Files
	 * (x86)\Windows Media Player\wmplayer.exe (Solo se non e' installa VLC)
	 * 
	 * MAC_OS: - ...
	 * 
	 * UNIX: - ...
	...
	public MoviePlayer() {
		...
		if (OS.contains("win")) {
			...
			if (pl.exists()) // ...se installato lo uso per riprodurre i film
				this.player = "\"C:\\Program Files (x86)\\VideoLAN\\VLC\\vlc.exe\""; // basta inserire il PATH di un
																						// player qualsiasi
			else // ...se non installato uso il player di sistema
				this.player = "\"C:\\Program Files (x86)\\Windows Media Player\\wmplayer.exe\""; // possibili problemi
																									// per la
																									// riproduzione di
																									// file con
																									// estensione ".mkv"
	...
	/**
	 * Permette di avviare la riproduzione di un file video. Se e' gia' in
	 * riproduzione un file video, questo viene arrestato e si riproduce il "nuovo"
	 * file video situato al PATH indicato come parametro
	 * 
	 * @param moviePath
	 *            - PATH del file video da riprodurre
	 * @param title
	 *            - Titolo del file video da riprodurre
	 */
	public void play(String moviePath, String title) {
		if (moviePath != null) {
			this.moviePath = moviePath;
			this.title = title;
			if (playing) { // se si sta gi� guardando un file video, lo arresto e avvia una nuova
							// riproduzione
				p.destroy();
				try {
